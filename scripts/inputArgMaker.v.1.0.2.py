...
def main():
    ...
    parser.add_argument('-p', '--pairedend', required=True, type=str, help='Are your samples paired end? YES/NO')
    ...
    if args.pairedend in ('True', 'true', 'TRUE', 'Yes', 'yes', 'y', 'Y', 't', 'T'):
       #input_file.write("sampleID\tforward\treverse\tsqtype\tminlength\tphred\tplatform\tfilesize\n")
       ...
       for keys in pairedEndKeys:
           input_file.write(keys + '\t' + fileList[keys]['forward'] + '\t' + fileList[keys]['reverse'] + '\t' + fileList[keys]['flag'] + '\t' + args.minlength + '\t' + args.phred + '\t' + args.pl + '\t' +  fileList[keys]['size'] + '\n')
    else:
       #print("You are running the singl end version\n")
       #input_file.write("sampleID\tforward\tsqtype\tminlength\tphred\tplatform\tfilesize\n")
       fileList=fileListRaw
       for files in fileList:
           sample, size = str(files[1]), str(files[0])
           baseName = return_sample_name(sample)
           input_file.write(baseName + '\t' + sample + '\t' + "SE" + '\t' +  args.minlength + '\t'  + args.phred + '\t' + args.pl + '\t' + size + '\n')
    input_file.close()
        ...
        #intiate keys
         ...
         for fileLine in fileList:
             if uniqueSamples in fileLine[1]:
                if 'r1' in fileLine[1]:
                    sampleDictionary[uniqueSamples]['forward'], sampleDictionary[uniqueSamples]['size'], sampleDictionary[uniqueSamples]['flag'] = fileLine[1], str(fileLine[0]), "PE"
                elif 'r2' in fileLine[1]:
                    sampleDictionary[uniqueSamples]['reverse'], sampleDictionary[uniqueSamples]['size'], sampleDictionary[uniqueSamples]['flag'] = fileLine[1], str(fileLine[0]), "PE"
